...
class Serial(CommonModbusFunctions):
    def __init__(self,
        ...
        # K230: UART pins already configured by FPIOA, no tx/rx arguments needed
    ...
    def _uart_any(self):
        """K230: 模拟 UART.any() 而无需不必要消耗。"""
        if len(self._uart_buffer) > 0:
            return len(self._uart_buffer)
    ...
    def _uart_read(self) -> bytearray:
        ...
        for x in range(1, 120):
            avail=self._uart_any()
            if avail>0:
                if len(self._uart_buffer) > 0:
                    response.extend(self._uart_buffer)
                    self._uart_buffer = bytearray()
                else:
                    response.extend(self._uart.read() or bytearray())
                # WiPy only
        ...
        while (time.ticks_diff(time.ticks_us(), start_us) <= timeout):
            ...
            if avail>0:
                ...
                while time.ticks_diff(time.ticks_us(), last_byte_ts) <= self._inter_frame_delay:
                    ...
                    # r = self._uart.read()
                    if len(self._uart_buffer) > 0:
                        received_bytes.extend(self._uart_buffer)
                        self._uart_buffer = bytearray()
                    else:
                        r=self._uart.read()
